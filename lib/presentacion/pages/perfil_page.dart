...
// ¡Mira mamá, sin Firebase! Pura arquitectura limpia.
import 'package:unimet_marketplace/domain/cubits/profile_cubit.dart';

class PerfilPage extends StatelessWidget {
  const PerfilPage({super.key});

  // Envolvemos la página original en un BlocProvider para inyectarle el Cubit
  @override
...
// Esta es tu página original, convertida en un StatefulWidget para las ventanitas
...
class _PerfilPageViewState extends State<_PerfilPageView> {
  ...
  // --- LAS FUNCIONES AHORA LE HABLAN AL CUBIT ---
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              // Le pedimos al Cubit que lo haga
              bool exito = await context.read<ProfileCubit>().actualizarTelefono(telefonoCtrl.text);
              if (!mounted) return;
              Navigator.pop(dialogContext); // Cerramos la ventanita
              if (exito) {
                _lanzarMensaje("Teléfono actualizado", Colors.green);
              } else {
                _lanzarMensaje("Error al actualizar teléfono", Colors.red);
              }
            },
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text("Esta solicitud será enviada a un administrador para su verificación.", style: TextStyle(fontSize: 13, color: Colors.black54)),
            const SizedBox(height: 20),
            DropdownButtonFormField<String>(
              isExpanded: true,
              decoration: const InputDecoration(labelText: 'Nueva Carrera', border: OutlineInputBorder()),
              items: opcionesCarrera.map((carrera) => DropdownMenuItem(value: carrera, child: Text(carrera, overflow: TextOverflow.ellipsis))).toList(),
          ...
          ElevatedButton(
            onPressed: () async {
              if (nuevaCarrera != null) {
                ...
                exito ? _lanzarMensaje("Solicitud enviada", Colors.blue) : _lanzarMensaje("Error al enviar", Colors.red);
              }
            },
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (!mounted) return;
              Navigator.pop(dialogContext);
              exito ? _lanzarMensaje("Nombre actualizado", Colors.green) : _lanzarMensaje("Error al actualizar", Colors.red);
            },
            child: const Text("Guardar"),
          ),
        ],
  ...
  void _funcionCambiarFoto() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton.icon(icon: const Icon(Icons.photo_library), label: const Text("Galería"), onPressed: () { Navigator.pop(context); _lanzarMensaje("Abrir Galería (Próximamente)", Colors.blue); }),
          TextButton.icon(icon: const Icon(Icons.camera_alt), label: const Text("Cámara"), onPressed: () { Navigator.pop(context); _lanzarMensaje("Abrir Cámara (Próximamente)", Colors.blue); }),
        ],
      ),
    );
  }
  ...
  void _funcionPassword() async {
    ...
    exito ? _lanzarMensaje("Correo enviado para restablecer clave", Colors.blueAccent) : _lanzarMensaje("Error al enviar correo", Colors.red);
  }
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (exito) {
                ...
                Navigator.pop(dialogContext);
                _lanzarMensaje("Inicia sesión de nuevo por seguridad antes de eliminar", Colors.red);
              }
            },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // MAGIA AQUÍ: BlocBuilder escucha al Cubit
      body: BlocBuilder<ProfileCubit, ProfileState>(
        builder: (context, state) {
          if (state is ProfileLoading) {
            return const Center(child: CircularProgressIndicator());
          } else if (state is ProfileError) {
            return Center(child: Text(state.mensaje, style: const TextStyle(color: Colors.grey)));
  ...
  Widget _buildUserCard(Map<String, dynamic> userData, String? photoURL) {
    ...
    String especialidad = rol == 'Estudiante' ? (userData['carrera'] ?? "") : (userData['departamento'] ?? "");
    String telefono = userData['telefono'] ?? "Sin teléfono";
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text("$especialidad\n$rol", style: const TextStyle(fontSize: 12, color: Colors.grey), textAlign: TextAlign.center),
          const Divider(height: 30),
          _infoLine(Icons.phone_outlined, telefono),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildOption(Icons.person_outline, "Cambiar Nombre", () => _funcionCambiarNombre(nombre)),
          const Divider(height: 1, color: Colors.black12),
          _buildOption(Icons.phone_android, "Cambiar Teléfono", () => _funcionCambiarTelefono(telefono)),
          
          if (rol == 'Estudiante') ...[
            const Divider(height: 1, color: Colors.black12),
            _buildOption(Icons.school_outlined, "Solicitar Cambio de Carrera", () => _funcionCambiarCarrera(especialidad)),
          ],
